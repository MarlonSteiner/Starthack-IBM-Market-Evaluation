...
const DraftingArea = ({
    ...
    const draftedArticles = articles.filter(article => draftingArticleIds.includes(article.id));
    ...
    const handleSendClick = () => {
        if (compiledDraft) {
            onSendForReview(compiledDraft, uniqueSourceObjects);
        }
    };

    return (
        <div className="bg-white p-6 border border-slate-200 rounded-lg shadow-md">
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Zusammengestellter Entwurf (KI-unterstützt)</h3>
            <textarea
                value={compiledDraft}
                ...
                className="w-full h-64 p-3 bg-slate-100 border border-slate-200 rounded-md text-slate-800 text-sm leading-relaxed focus:outline-none focus:ring-2 focus:ring-slate-500"
            />
                ...
                <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {uniqueSourceObjects.map(source => (
                            <span key={source.name} className="px-2.5 py-1 text-xs font-semibold rounded-md bg-slate-100 text-slate-700">
                                {source.name}
                            </span>
                        ))}
                    </div>
